...
for compare = START_ENTRY:STOP_ENTRY
    ...
    rl_lbp_reference = data{compare,14};                     % RL lbp
    mac_lbp_reference = data{compare,15};                    % MAC lbp
...
% for LBP find range and fill matches array
if strcmp(test_method,'LBP')
    matches_array_sorted = sort(matches_array(:));
    max = matches_array_sorted((STOP_ENTRY - START_ENTRY)^2 + STOP_ENTRY - START_ENTRY);
    matches_array(matches_array > max) = max;
    matches_array(matches_array == -1) = max;
    matches_array = 100-(matches_array./max.*100);
    matches_array(1:1+size(matches_array,1):end) = -1;
end

% save to matrix file
save('matches_array_GIVE_ME_A_NAME.mat','matches_array');
